...
def CrossEntropyLoss_smoothing(label_smoothing=0.05):
    return nn.CrossEntropyLoss(label_smoothing=label_smoothing);

def train_model(net, train_x, train_y, val_x, val_y, mode, epochs=50, batch_size=64):
    loss_func = [nn.CrossEntropyLoss,CrossEntropyLoss_smoothing]
    train_dataset = TensorDataset(train_x, train_y)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    criterion = loss_func[mode]()
...
model = TextTransformer(embedding, dim=dim, num_heads=8, num_layers=3, output_dim=5, dropout_rate=0.5).to(device)
acc = []
loss_func_name = ['CrossEntropyLoss','CrossEntropyLoss_smoothing']
for i in range(2):
    train_model(model, x_train, y_train_t, x_val, y_val_t, mode=i, epochs=50, batch_size=50)
    a = evaluate(model, x_val, y_val_t)
    acc.append(a);
plt.plot(loss_func_name, acc, marker='o', linestyle='-', color='b')
plt.xlabel('loss_func_name')
plt.ylabel('accuracy')
plt.title('loss_func')
plt.savefig("overview_loss_func.png")
