...
acc = []
sizes_name = ['1', '1,2','2,3','2,3,4','3,4,5']
sizes = [[1],[1,2],[2,3],[2,3,4],[3,4,5]]
for i in range(5):
    model = TextCNN(embedding, dim=dim,filter_size=sizes[i],output_dim=5, ).to(device)
    train_model(model, x_train, y_train_t, x_val, y_val_t, epochs=50, batch_size=50)
    a = evaluate(model, x_val, y_val_t)
    acc.append(a);
plt.plot(sizes_name, acc, marker='o', linestyle='-', color='b')
plt.xlabel('filter_size')
plt.ylabel('accuracy')
plt.title('filter_size')
plt.savefig("overview_filter_size.png")
